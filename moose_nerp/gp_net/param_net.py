...
neuron1pop=neur_distr(neuronname='proto', spacing=grid,percent=0.7) #Table 2 Hernandez Parvabinum+ Neurons and arky Neurons 2015
#calculated from percent composition of PV+=proto, Npas1+/FoxP2+=Npas, Lhx6=arky
neuron2pop=neur_distr(neuronname='Npas', spacing=grid,percent=0.3)
...
#tt_STN = Table('tt_Str_SPN','gp_net/SPN_lognorm',syn_per_tt=2)
#description of intrinsic inputs
ConnSpaceConst=300e-6 #used to calculate likelyhood of two neurons connecting
proximal_distr=dend_location(mindist=0e-6,maxdist=200e-6,postsyn_fraction=0.5) #specifies where on the neuron synapses are made
#distal_distr=dend_location(mindist=100e-6,maxdist=1000e-6,half_dist=300e-6,steep=1)
...
#ext1_neur1post=ext_connect(synapse='gaba',pre=tt_Str_SPN,post='proto', dend_loc=STN_distr)# Corbit Whalen 2016 Table 2 connectiv#ext1_neur1post=ext_connect(synapse='gaba',pre=tt_Str_SPN,post='proto', dend_loc=Str_distr)#ext1 = Str
